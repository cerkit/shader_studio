...
struct GeminiClient {
    ...
    func generateShader(prompt: String, apiKey: String) async throws -> String {
        ...
        let redactedURL = url.absoluteString.replacingOccurrences(of: apiKey, with: "[GEMINI_API_KEY]")
        print("GeminiClient: Sending request to \(redactedURL)")
        
        // Debug: Print CURL command
        if let jsonData = try? JSONEncoder().encode(requestBody), let jsonString = String(data: jsonData, encoding: .utf8) {
            ...
            print("curl -X POST \"\(redactedURL.replacingOccurrences(of: "[GEMINI_API_KEY]", with: "${GEMINI_API_KEY}"))\" \\")
        ...
        guard let httpResponse = response as? HTTPURLResponse, httpResponse.statusCode == 200 else {
            if let errorText = String(data: data, encoding: .utf8) {
                ...
                throw GeminiError(message: "API Error \( (response as? HTTPURLResponse)?.statusCode ?? 0 ): \(errorText)")
            }
